...
public class EnemySpawner : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            StartCoroutine(SpawnEnemy(spawnTime, enemyPrefab, enemyCount)); // Start spawning enemies every 3 chunks
            enemyCount = 0; // Reset enemy count after spawning
        }
        else if (chunkIndex % 4 != 0 && chunkIndex != 0)
        {
            enemyCount = 5; // Reset enemy count if not in the right chunk
            StopAllCoroutines(); // Stop spawning enemies if not in the right chunk
        }
    }

    // Update is called once per frame
    private IEnumerator SpawnEnemy(float spawnTime, GameObject enemy, int maxEnemies)
    {
        if (maxEnemies <= 0)
        {
            yield break; // Stop spawning if max enemies reached
        }
        else
        {
            ...
            StartCoroutine(SpawnEnemy(spawnTime, enemy, maxEnemies - 1)); // Decrease the count of max enemies
        }
    }
}
